    ...
    if (message.text).isdigit():
        if 0 < int(message.text) < (max_take + 1):
            ...
            if model.check_win(total):
                await bot.send_message(player.id, f'Ура! Ты, {player.first_name}, выиграл!')
                return
...
async def set_total_candies(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, f'Максимально количество конфет теперь - {count}')
...
async def set_level(message: types.Message):
    ...
    if count == 1:
        await bot.send_message(message.from_user.id, f'Выбран уровень: hard')
    else:
        await bot.send_message(message.from_user.id, f'Выбран уровень: easy')
